...
class LibraryActionsTests(unittest.TestCase):
    def setUp(self) -> None:
        self.recordings = [
            BBBRecording(
                ...
                imported_at="2025-01-01T10:00:00+00:00",
    ...
    def test_filter_and_sort_recordings(self) -> None:
        ...
        recent_rec = BBBRecording(
            meeting_id="m-recent",
            source_url="https://bbb.test/p?m=recent",
            title="Recent Lecture",
            imported_at=now.isoformat(),
            audio_video_url="https://bbb.test/a-recent.mp4",
            screen_video_url=None,
            slides=[],
        )
        all_recs = [*self.recordings, recent_rec]
        today_filtered = filter_and_sort_recordings(all_recs, date_filter="today")
        self.assertEqual(len(today_filtered), 1)
        self.assertEqual(today_filtered[0].meeting_id, "m-recent")

        week_filtered = filter_and_sort_recordings(all_recs, date_filter="7_days")
        self.assertEqual(len(week_filtered), 1)

        month_filtered = filter_and_sort_recordings(all_recs, date_filter="30_days")
        self.assertEqual(len(month_filtered), 1)
    ...
    def test_list_and_empty_trash(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            base_dir = Path(temp_dir)
            lib_path = base_dir / "library.json"
            save_library(self.recordings, path=lib_path)
            ...
            self.assertEqual(list_trash(base_dir), [])

            # Move to trash
            move_to_trash(lib_path, "m-1", base_dir)
            trash_items = list_trash(base_dir)
            self.assertEqual(len(trash_items), 1)
            self.assertEqual(trash_items[0]["meeting_id"], "m-1")

            # Empty trash
            deleted_count = empty_trash(base_dir)
            self.assertEqual(deleted_count, 1)
            self.assertEqual(list_trash(base_dir), [])
            self.assertFalse((base_dir / "trash" / "m-1").exists())
